...
class ApiClient {
  ...
  Future<T?> postData<T>({
    ...
    try {
      ...
      // }
      // log("response is=-=-11  $responseString");
      dynamic map = json.decode(responseString);
      if (response.statusCode == 200 ||
          response.statusCode == 201 ||
          response.statusCode == 202) {
        // log("response statusCode is=-=-22 $responseString");
        return builder(responseString);
      } else {
        // log("response.reasonPhrase=-=-22 ${response.reasonPhrase} ");
        DialogServiceV1().showSnackBar(
            content: map['data']["message"] ??
                map["message"] ??
